    ...
    rects.enter().append('rect')
      ...
      .on('click', function selectBar(datum) {
        d3Selection.selectAll('.bar')
          .attr('fill', d => textureShape(svg, colorScale(d[1]),
          d3ScaleChromatic.interpolateBlues));
        const bar = d3Selection.select(this);
        bar.attr('fill', textureShape(svg, colorScale(datum[1]),
          d3ScaleChromatic.interpolateReds));
        thisBarChart.worldMap.updateMap(datum);
        thisBarChart.infoPanel.updateInfo(datum);
      });
