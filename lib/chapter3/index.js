...
export function axisDemos() {
  ...
  const axes = [
    ...
    d3Axis.axisTop().scale(xScale)
      .ticks(5),
    d3Axis.axisBottom().scale(xScale)
      .tickSize(10, 5, 10),
    d3Axis.axisTop().scale(xScale)
      .tickValues([0, 20, 50, 70, 100])
      .tickFormat((datum, index) => ['a', 'e', 'i', 'o', 'u'][index]),
  ];
...
export function drawPaths() {
  ...
  pathContainer.append('path')
    .datum(sins)
    .attr('d', lineAlgorithm)
    .attr('stroke', 'steelblue')
    .attr('stroke-width', 2)
    .attr('fill', 'none');
  pathContainer.append('path')
    .datum(sins)
    .attr('d', lineAlgorithm.curve(d3Shape.curveNatural))
    .attr('stroke', 'black')
    .attr('stroke-width', 2)
    .attr('fill', 'none');
  const pathContainer2 = chart.container.append('g')
    .attr('transform',
      ...
      ${chart.margin.top})`);
  const areaAlgorithm = d3Shape.area()
    .x(datum => xScale(datum[0]))
    .y0(chart.height / 2)
  ...
  pathContainer2.append('path')
    .datum(sins)
    .attr('d', areaAlgorithm)
    .attr('fill', 'steelblue')
    .attr('fill-opacity', 0.4);
  pathContainer2.append('path')
    .datum(sins)
    .attr('d', lineAlgorithm.curve(d3Shape.curveBasis))
    .attr('stroke', 'steelblue')
    .attr('stroke-width', 2)
    .attr('fill', 'none');
  ...
  const arcAlgorithm = d3Shape.arc()
    .outerRadius(100)
    .innerRadius(50)
  ...
  const ribbonAlgorithm = d3Chord.ribbon()
