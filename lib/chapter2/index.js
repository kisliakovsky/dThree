...
export async function renderSvgStuff() {
  ...
  chart.container.selectAll('ellipse, circle')
    .attr('transform', `
    translate(${CIRCLE_CENTER_SHIFT}, 0) 
    scale(${CIRCLE_SCALE}) 
    translate(-${(CIRCLE_CENTER[0] + CIRCLE_CENTER_SHIFT) * CIRCLE_SCALE - CIRCLE_CENTER[0]}, 0) 
    rotate(-45, ${CIRCLE_CENTER[0] / CIRCLE_SCALE}, ${CIRCLE_CENTER[1] / CIRCLE_SCALE})`);
}
