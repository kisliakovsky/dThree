...
const scaleExample = d3Scale.scaleOrdinal()
...
function quantitativeScales(chart) {
  ...
  const logXScale = d3Scale.scaleLog()
    .domain(d3Array.extent(data.filter((datum) => {
      if (datum > 0) {
        return datum;
      } else {
        return 0;
    ...
    .range([0, chart.width]);
  const logAlgorithm = lineAlgorithm.x((datum) => {
    if (datum > 0) {
      return logXScale(datum);
    } else {
      return 0;
    }
  })
...
function generatePrimes(count) {
  const primes = [];
  for (const prime of getPrimes(count)) {
    primes.push(prime);
  }
  return primes;
}
...
const ulamSpiral = ((enabled) => {
  if (!enabled) return;
  const chart = chartFactory();
  const dotAlgorithm = d3Shape.symbol().type(d3Shape.symbolCircle)
    .size(3);
  const center = 400;
  const cellSideLen = 2;
  const positioner = (coord, cellSideLength) => center + (coord * cellSideLength);
