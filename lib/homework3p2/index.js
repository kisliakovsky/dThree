...
const elders = ['Group', 'Round of Sixteen', 'Quarter Finals', 'Semi Finals'];

function determineResult(teamObjects) {
  let elder = 'Group';
  for (const teamObject of teamObjects) {
    const result = teamObject.Result;
    const win = teamObject.Wins;
    if (result === 'Winner' || (result !== 'Group' && win === 0)) {
      return result;
    } else if (elders.indexOf(result) > elders.indexOf(elder)) {
      elder = result;
    }
  }
  return elder;
...
function aggregateData(csvData) {
  return d3Collection.nest()
    ...
    .rollup((teamObjects) => {
      ...
      const result = determineResult(teamObjects);
      return {
        'Goals Made': made,
 ...
 */
d3Request.csv('data/fifa-matches.csv', (error, data) => {
  /**
